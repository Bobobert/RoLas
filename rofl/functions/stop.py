from rofl.utils.utils import timeToStop, timeFormatedS
...
def testEvaluation(config, agent, trainResults = None):
    ...
    # Saving time
    trainResults["time_execution"] += [timeFormatedS()]
    # Evaluating time
    newTime, stopTime = timeToStop(trainResults["time_elapsed"], config["train"].get("max_time", None))
    trainResults["time_elapsed"] = newTime
    ...
    return trainResults, (stopTime or stopPerE or stopPerM)